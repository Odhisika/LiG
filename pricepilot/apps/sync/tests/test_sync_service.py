...
from apps.pricing.services import PricingRuleService
...
class TestDeletion:
    def test_delete_removes_store_row_and_clears_pointers(self, user, supplier, lig_store):
        seed_category("UPS", "ups")
        product = make_product(user, supplier)

        from apps.sync.services import StoreSyncService

        seeded = StoreSyncService.sync_product(product)
        product.refresh_from_db()
        assert LiGProduct.objects.filter(id=seeded["lig_product_id"]).exists()

        result = StoreSyncService.delete_product(product)
        product.refresh_from_db()

        assert result["action"] == "deleted"
        assert result["deleted"] is True
        assert not LiGProduct.objects.filter(id=seeded["lig_product_id"]).exists()
        assert product.store_product_id is None
        assert product.store_synced_at is None

    def test_delete_clears_sku_match_too(self, user, supplier, lig_store):
        seed_category("UPS", "ups")
        LiGProduct.objects.create(
            product_name="APC Easy 1000VA UPS",
            slug="apc-easy-1000va-ups",
        ...
        assert result["action"] == "deleted"
        assert not LiGProduct.objects.filter(sku="SKU-EXISTING").exists()
        assert product.store_product_id is None

    def test_delete_is_noop_when_never_synced(self, user, supplier, lig_store):
        from apps.sync.services import StoreSyncService
    ...
    def test_delete_skips_when_disabled(self, user, supplier, lig_store):
        from apps.sync.services import StoreSyncService
...
class TestDeletion:
    def test_delete_product_hard_deletes_lig_row(self, user, supplier, lig_store):
        seed_category("UPS", "ups")
        product = make_product(user, supplier)
        ...
        seeded = StoreSyncService.sync_product(product)
        lig_id = seeded["lig_product_id"]
        product.refresh_from_db()
        assert product.store_product_id == lig_id
        assert LiGProduct.objects.filter(id=lig_id).exists()
        ...
        assert product.store_product_id is None
        assert product.store_synced_at is None
        assert not LiGProduct.objects.filter(id=lig_id).exists()


class TestDashboardDrivenRepricing:
    def test_pricing_rule_update_reprices_and_syncs_existing_store_row(
        self, user, supplier, lig_store
    ):
        seed_category("UPS", "ups")
        rule = PricingRule.objects.create(owner=user, name="Markup")
        ...
        seeded = StoreSyncService.sync_product(product)
        lig = LiGProduct.objects.get(id=seeded["lig_product_id"])
        assert lig.price == Decimal("110.00")

        updated_rule = PricingRuleService.update(
            user,
            rule.id,
            {
                "name": "Markup",
